...
class Gpt(nn.Module):
    ...
    def compute_token_sizes(self, parameter_sizes, parameter_names, num_frequencies):
        ...
        input_parameter_sizes.extend([scalar_token_size, [512]])
        input_parameter_names.extend(['timestep_embedding', 'text_embedding'])
    ...
    def attention_fusion(self, embeddings):
        """
        Input:
            embeddings: shape [batch_size, num_sentences, feature_dim]
        Output:
            fused_vector: shape [batch_size, feature_dim]
        """
    ...
    def forward(self, x, t, cond):
        ...
        cond: (N, 512) tensor of text condition feature
        ----------------------------------------------
        returns: (N, D) tensor of denoised updated parameters
        ----------------------------------------------
        """
        t_embedding = self.scalar_embedder(t)
        # cond_embedding = self.attention_fusion(cond)
        inp = [x, t_embedding, cond]
        inp = torch.cat(inp, 1)
