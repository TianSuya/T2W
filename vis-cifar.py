...
def vis_loss(g_nets, ds, x0_net_paths, device):
    ...
    with open('./data_gen/cifar100/fusion_data.json', 'r') as f:
        selected_point = json.load(f)

    clip_model, _ = clip.load("ViT-B/32", device=device)
    text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") for c in selected_point[0]['selected_classes']]).to(device)
    ...
    init_net_sd = torch.load('./data_gen/cifar100/checkpoints/base-fusion.pt', map_location='cpu')
    ...
    a_net_sd = torch.load('./data_gen/cifar100/fusion/modelA.pt', map_location='cpu')
    b_net_sd = torch.load('./data_gen/cifar100/fusion/modelB.pt', map_location='cpu') #some other net
...
def vis(cfg):
    ...
    x0_net_paths = []
    count = 0
    with open('./data_gen/cifar100/selected_point.json','r') as f:
    ...
    # }
    # torch.save(save_dict, './cifar_fusion.pt')
    # with open('./cifar_denoise.json', 'w') as f:
    #     json.dump(result, f)
