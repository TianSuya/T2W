...
import scienceplots
import json
from data_gen.imagenet.generate_dataset import CLIPAdapter, TinyImageNet
...
def eval_generate_model(
    ...
    for i in range(len(selected_classes_set)):
        test_dataset = TinyImageNet(
            ...
            class_names=selected_classes_set[i]
        )
...
def vis_loss(g_nets, ds, x0_net_paths, device):
    ...
    init_net_sd = torch.load('./data_gen/imagenet/checkpoints/base.pt', map_location='cpu')
    ...
    b_net_sd = torch.load('./data_gen/imagenet/checkpoints/10/task.pt', map_location='cpu') #some other net
...
def vis(cfg):
    ...
    G_path = '/data/bowen/Text2Weight/results/imagenet_classifier/checkpoints/best.pt'
    ...
    x0_net_paths = []
    count = 0
    with open('./data_gen/imagenet/selected_point.json','r') as f:
    ...
    torch.save(save_dict, './imagenet_fusion.pt')
    # with open('./imagenet_denoise.json', 'w') as f:
    #     json.dump(result, f)
