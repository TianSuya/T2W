...
base_dir = '/data/bowen/Text2Weight/data_gen/caltech256/checkpoints'
with open('./classes_map.json','r') as f:
    classes_map = json.load(f)
data_range = [i for i in range(12000)]
dir_names = os.listdir(base_dir)
results = []
clip_model, _ = clip.load("ViT-B/32", device='cuda:5')
...
for item in tqdm.tqdm(dir_names):
    ...
    if os.path.exists(file_path):
        ...
        text_inputs = torch.cat([clip.tokenize(classes_map[c]) for c in selected_classes])
        text_features = clip_model.encode_text(text_inputs.cuda(5))
...
with open('train-natural.json','w') as f:
    json.dump(train_dataset, f, indent=4)

with open('val-natural.json','w') as f:
